...
# --- 2. 로또 API 및 등수 계산 함수 (보안 및 안정성 강화) ---
...
def generate_recommendations():
    ...
    for i in range(confirmed_no, confirmed_no - 50, -1):
        ...
        if len(recent_history) >= 20: break
...
def generate_dynamic_comment(best_numbers):
    ...
    detail = "황금 밸런스 조합입니다." if 120 < total_sum < 160 else f"총합 {total_sum}의 최적 조합입니다."
    return f"{random.choice(intros)} {detail} 강력한 당첨 신호가 감지되었습니다."

# --- 5. 당첨 확인 및 업데이트 로직 (호환성 높은 where 방식 사용) ---
def check_winning_status():
    # .filter 대신 .where 를 사용하여 라이브러리 버전 호환성 해결
